...
import 'package:presentation/provider/tv_show_detail_notifier.dart';
...
void main() {
  ...
  group('get tv detail', () {
    ...
    test('should change tvdetail data after fetching data through usecase',
        () async {
      when(getDetail(1)).thenAnswer((_) async => const Right(testTvShowDetail));
  ...
  group('watchlist', () {
    ...
    test('remove from watchlist should change message', () async {
      ...
      when(getWatchlistStatus(1)).thenAnswer((_) async => false);
      when(removeWatchlist(1)).thenAnswer((_) async => const Right('success'));
